# Building and Deploying OtpAuthentication
...
dotnet add package OtpAuthentication --version 1.0.0 --source LocalFeed
```
...
```xml
<ItemGroup>
  <PackageReference Include="OtpAuthentication" Version="1.0.0">
    <Source>C:\path\to\OtpAuthentication\artifacts\OtpAuthentication.1.0.0.nupkg</Source>
  </PackageReference>
</ItemGroup>
```
...
2. Click **Create**
3. Provide a key name (e.g., "OtpAuthentication")
4. Select **Push** scope
5. Select packages or glob pattern (e.g., `OtpAuthentication*`)
6. Copy the generated API key

### Step 2: Publish

```bash
dotnet nuget push ./artifacts/OtpAuthentication.1.0.0.nupkg \
...
https://www.nuget.org/packages/OtpAuthentication
...
unzip OtpAuthentication.1.0.0.nupkg -d extracted/
...
```
error: Response status code does not indicate success: 409 (Conflict - The feed already contains 'OtpAuthentication' version '1.0.0'.)
```
